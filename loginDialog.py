...
class loginDialog(QDialog):
    def __init__(self, username="", password=""):
        ...
        self.domainEdit.setCurrentText(Subject.settings.value("loginDialogDefaultDomain", defaultValue=".digitalesregister.it"))
        self.domainEdit.textHighlighted.connect(lambda x: self.domainEdit.setCurrentText(schools.schools[x]))
        self.domainEdit.textActivated.connect(lambda x: self.domainEdit.setCurrentText(schools.schools[x]))
        ...
        self.semesterSpinbox.setCurrentIndex(int(Subject.settings.value("loginDialogDefaultSemester", defaultValue=0)))
    ...
    def setMessage(self, message, state=0):
        p = QPalette()
        if state == 1: p.setColor(QPalette.WindowText, self.palette().windowText().color())
        if state == 2: p.setColor(QPalette.WindowText, Qt.red)
        if state == 3: p.setColor(QPalette.WindowText, Qt.green)
    ...
    def accept(self) -> None:
        ...
        Subject.settings.setValue("loginDialogDefaultDomain", self.domainEdit.currentText())
        Subject.settings.setValue("loginDialogDefaultSemester", self.semesterSpinbox.currentIndex())
        return super().accept()
    ...
    def requestThread(self):
        ...
        try:
            ...
            data = {"username": self.userEdit.text(),"password": self.passwordEdit.text()}
            self.setMessage("sending Request", state=1) 
            resp = requests.post(url, json=data)
            print(resp.json())
            if resp.json()["error"] is not None:
                self.setMessage(f"Auth failed. Server responce: " + str(resp.json()), state=2)
            else:
